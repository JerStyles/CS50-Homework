...
void tabulate(void)
{
    ...
    {
        int rank = 0;
        int candidate_num = preferences[voterIndex][rank];

        while (candidates[candidate_num].eliminated)
        {
            candidate_num = preferences[voterIndex][rank++];
        }
...
bool is_tie(int min)
{
    ...
    if (sameVotes_count == validCandidates)
    {
        return true;
    }
