...
void sort_pairs(void)
{
    // Use a bubble sorting algorithm to sort pairs by strength of victory
    for (int i = 0; i < pair_count - 1; i++)
    {
        for (int j = 0; j < pair_count - 2; j++)
        {
            ...
            // Swap pairs if the strength of victory is lower
...
void lock_pairs(void)
{
    for (int i = 0; i < pair_count; i++)
    {
        ...
        // Check if creating a cycle will occur before locking the pair
...
bool create_cycle(int winner, int loser)
{
    if (winner == loser)
    {
        return true; // Reject self-loops
    }

    // Create a copy of the locked array to track temporary locks
    ...
    // Simulate the lock of the current pair
    temp_locked[winner][loser] = true;

    // Check if there is a path from the loser to the winner in the directed graph
    for (int i = 0; i < candidate_count; i++)
    {
